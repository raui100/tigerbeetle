...
/// Returns the size of an exhaustive enum.
fn enum_size(EnumOrUnion: type) u8 {
...
/// value or be sparse to a continuous index that fits within enum_size().
...
pub const EventTiming = union(Event.Tag) {
    ...
    pub const slot_limits = std.enums.EnumArray(Event.Tag, u32).init(.{
        .replica_commit = enum_size(CommitStage.Tag),
        .replica_aof_write = 1,
        .replica_aof_checkpoint = 1,
        .replica_sync_table = 1,
        .replica_request = enum_size(Operation),
        .replica_request_execute = enum_size(Operation),
        .replica_request_local = enum_size(Operation),
        .compact_beat = enum_size(TreeEnum),
        .compact_beat_merge = enum_size(TreeEnum),
        .compact_manifest = 1,
        .compact_mutable = enum_size(TreeEnum),
        .compact_mutable_suffix = enum_size(TreeEnum),
        .lookup = enum_size(TreeEnum),
        .lookup_worker = enum_size(TreeEnum),
        .scan_tree = enum_size(TreeEnum),
        .scan_tree_level = enum_size(TreeEnum),
        .grid_read = 1,
        .grid_write = 1,
        .metrics_emit = 1,
        .storage_read = enum_size(Zone),
        .storage_write = enum_size(Zone),
        .client_request_round_trip = enum_size(Operation),
    });
...
pub const EventMetric = union(enum) {
    ...
    pub const slot_limits = std.enums.EnumArray(Tag, u32).init(.{
        .table_count_visible = enum_size(TreeEnum),
        .table_count_visible_max = enum_size(TreeEnum),
        .replica_status = 1,
        .replica_view = 1,
        .replica_log_view = 1,
        .replica_op = 1,
        ...
        .replica_messages_in = enum_size(Command),
        .replica_messages_out = enum_size(Command),
        .journal_dirty = 1,
        .journal_faulty = 1,
